...
function renderFinanceManager() {
  ...
  // Считаем расходы (из общака)
  // Пока нет отдельного API для расходов, используем заглушку
  const totalExpenses = 0; // TODO: добавить API для расходов
  ...
  return `
        ...
        <div style="margin-top:20px">
            <button class="btn btn-primary btn-block" onclick="openAddPaymentModal()">➕ Зарегистрировать поступление</button>
        </div>
    `;
}
...
function renderFinanceAdmin() {
  ...
  // Считаем по каждому пользователю
  const users = ["u_002", "u_003", "u_004"]; // Максим, Андрей, Лев
...
function openAddPaymentModal() {
  ...
  openModal(`
        ...
        <div class="modal-actions">
            ...
            <button class="btn btn-primary" onclick="registerPayment()">Зарегистрировать</button>
        </div>
    `);
}

async function registerPayment() {
  const projectId = $("#paymentProject").value;
  const amount = parseInt($("#paymentSum").value);
